...
robloxRouter.get("/roblox/games/gamepasses", async (req, res) => {
  ...
  try {
    // Step 1: get game creator info so we can search catalog by creator
    ...
    let rootPlaceId: number | null = null;

    if (gameDetailsRes.ok) {
      const details = (await gameDetailsRes.json()) as {
        data?: Array<{ creator?: { id: number; type: string }; rootPlaceId?: number }>;
      };
      const game = details.data?.[0];
      creatorId = game?.creator?.id ?? null;
      creatorType = (game?.creator?.type ?? "User") as CreatorType;
      rootPlaceId = game?.rootPlaceId ?? null;
    }

    // Step 2: try catalog API (public) to find game passes by creator
    let passes: Array<{ id: number; name: string; displayName?: string; price?: number; isForSale?: boolean }> = [];

    if (creatorId) {
      const creatorTypeParam = creatorType === "Group" ? "Group" : "User";
      const catalogUrl = `https://catalog.roblox.com/v1/search/items?category=GamePass&creatorTargetId=${creatorId}&creatorType=${creatorTypeParam}&limit=30&sortOrder=Asc`;
      try {
        const catRes = await fetch(catalogUrl, { headers: ROBLOX_HEADERS });
        if (catRes.ok) {
          const catData = (await catRes.json()) as {
            data?: Array<{ id: number; name?: string }>;
          };
          if (catData.data?.length) {
            // Batch-fetch item details to get prices
            const ids = catData.data.map((i) => i.id);
            ...
            if (detailsRes.ok) {
              const detData = (await detailsRes.json()) as {
                data?: Array<{
                  id: number;
                  name?: string;
                  price?: number;
              ...
              // Filter to only items belonging to this universe
              const universeNum = Number(universeId);
              passes = (detData.data ?? [])
                .filter((item) => !item.universeId || item.universeId === universeNum)
                .map((item) => ({
                  id: item.id,
              ...
              // Fallback: use raw catalog data without prices
              passes = catData.data.map((i) => ({ id: i.id, name: i.name ?? "Game Pass" }));
            }
          }
        }
      } catch { /* non-fatal — fall through to empty */ }
    }

    // Also try the games API directly (may work for some games)
    if (passes.length === 0) {
      try {
        ...
        if (gpRes.ok) {
          const gpData = (await gpRes.json()) as {
            data?: Array<{ id: number; name: string; displayName?: string; price?: number; isForSale?: boolean }>;
          };
          passes = gpData.data ?? [];
        }
      } catch { /* ignore */ }
    ...
    if (passes.length === 0 && rootPlaceId) {
    ...
    // Fetch icons for all gamepasses
    ...
    try {
      ...
      if (thumbRes.ok) {
        ...
        for (const t of thumbData.data ?? []) {
          if (t.state === "Completed" && t.imageUrl) {
            iconMap[t.targetId] = `/api/roblox/proxy-image?url=${encodeURIComponent(t.imageUrl)}`;
          }
        }
      }
    } catch { /* non-fatal */ }

    const gamepasses: GamePass[] = passes.map((p) => ({
      id: p.id,
      name: p.displayName ?? p.name,
      price: p.price ?? null,
      isForSale: p.isForSale ?? false,
      iconUrl: iconMap[p.id] ?? null,
    }));

    gamepassCache.set(universeId, gamepasses);
    res.json({ gamepasses });
  } catch (err) {
    req.log.error(err);
    res.status(500).json({ error: "Internal server error" });
